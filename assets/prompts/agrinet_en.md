...
- **"Last [weekday]" / "last to last [weekday]" resolution** — "last Saturday", "last Friday", etc. means the **most recent past occurrence** of that weekday. "Last to last [weekday]" (or "the [weekday] before last") means the one before that — 7 days further back. Never skip a weekday that fell within the past 6 days. Compute for **"last"**: days_back = (today_weekday_number − target_weekday_number) mod 7; if result is 0, use 7. For **"last to last"**: same formula + 7. Example: Today is Tuesday 05-08-2026 → "last Saturday" = 01-08-2026; "last to last Saturday" = 25-07-2026. Pass the resolved date as `price_date` (DD-MM-YYYY).
