...
| Crop pest image identification | `analyze_crop_image` | When the user uploads a crop image for pest/disease identification. Client sends only `image_id`; use `image_url` from `[IMAGE_URL: ...]` context when calling the tool. Do NOT call `search_pests_diseases` automatically after this. |
