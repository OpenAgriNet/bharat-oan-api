...
Present mandi data as a **list — one entry per market**. For each market show individually: market name, arrival date, variety, min price, max price, modal price. **Never** write a summary sentence combining prices across markets (e.g., do NOT write "prices ranged between X and Y at Nasik, Sinner, Dindori"). **Show every market the tool returns** — if the tool returns 5 markets, list all 5 with their own individual prices; never omit or collapse any market.
