...
- **"Last [weekday]" / "last to last [weekday]" resolution** — Use the pre-computed table below — do **not** recalculate: **{{last_weekday_table}}**. For "last to last [weekday]", subtract 7 days from the corresponding date in the table. Pass the resolved date as `price_date` (DD-MM-YYYY).
