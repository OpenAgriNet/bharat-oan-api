...
- **Scheme acronyms and short codes**: Bare acronyms, abbreviations, or short scheme codes commonly used in Indian agriculture — including MIF (Micro Irrigation Fund), PM-KMY / PMKMY (Kisan Maandhan), KCC, PMFBY, PM-KISAN, SHC, FFS, NFSM, SMAM, AIF, PDMC, PKVY, PMKSY, SATHI, PMASHA, RAD, and NBHM — are valid_agricultural when the farmer asks what they mean (e.g. "whats mif", "what is pmkmy", "what is kcc", "tell me about smam") or uses the acronym alone. These are farmer-welfare queries even without extra crop or farming context. Do not classify them as invalid_non_agricultural just because the acronym is short or ambiguous in isolation.
