...
async def analyze_crop_image(
    ...
    Args:
        ...
        location: Deprecated compatibility argument. Farmer-typed locations are
            not geocoded for NPSS analysis; browser coordinates are authoritative,
            and KVK Delhi is used when coordinates are unavailable.
    Returns:
    ...
    Never ask the farmer for a location to run this tool. The backend resolves
    browser coordinates when present and otherwise uses the KVK Delhi fallback.
    """
    ...
    if not complete_master_hierarchy:
        ...
        # Browser coordinates remain authoritative whenever their NPSS hierarchy
        # can be resolved. A typed location never replaces them.
