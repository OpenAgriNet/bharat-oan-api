...
def _is_zero(val: Any) -> bool:
    """Return True if val is effectively zero (handles floating-point noise like -3.88e-16)."""
    try:
...
def _format_seed_search_response(data: dict[str, Any], crop_code: str | None = None) -> str:
    ...
    # Check for API-level failure in search context
    ...
    # ── Accumulate dealers (de-duped, zero-stock filtered) ──────────────────
    dealer_acc: dict[str, dict[str, Any]] = {}
    variety_lines: list[str] = []

    for p in merged:
        desc = p.get("descriptor") if isinstance(p.get("descriptor"), dict) else {}
        pname = str(desc.get("name") or "").strip() or "(unknown variety)"
        pcode = str(desc.get("code") or p.get("id") or "").strip() or "—"

        items = p.get("items") if isinstance(p.get("items"), list) else []
        variety_total_bags = 0

        for it in items:
            if not isinstance(it, dict):
                continue
            sections = _extract_seed_tag_sections(it.get("tags"))
            sm = _seed_detail_map(sections.get("seed-details") or [])
            bags = _num(sm.get("total-bags", 0), True)
            variety_total_bags += int(bags)

            for row in sections.get("dealer-list") or []:
                if isinstance(row, dict):
                    _accumulate_dealer(dealer_acc, row)

        if variety_total_bags > 0:
            variety_lines.append(f"- **{pname}** (`{pcode}`) — {variety_total_bags} bags in stock")

    # ── Build output ─────────────────────────────────────────────────────────
    lines: list[str] = []

    # Header
    lines.append(f"🌾 **Seed Availability — {location_str}**")
    lines.append("")

    # Varieties with stock
    if variety_lines:
        lines.append("**Available Varieties**")
        lines.extend(variety_lines)
        lines.append("")
    else:
        lines.append("No varieties with available stock found.")
        lines.append("")
    ...
    if active_dealers:
        lines.append("**Dealers with Stock**")
        lines.append("")
        for d in active_dealers.values():
            dist = d.get("district") or ""
            st = d.get("state") or ""
            place = ", ".join(x for x in (dist, st) if x) or "—"
            contact = d.get("contact_number") or "Not available"
            bags = d.get("bags", 0)
            q = float(d.get("quintals", 0.0))
            name = d.get("dealer_name") or "(unknown)"

            lines.append(f"**{name}**")
            lines.append(f" {place}")
            lines.append(f" {contact}")
            lines.append(f" {bags} bags ({q:g} quintals)")
            lines.append("")
    else:
        lines.append("No dealers with available stock found.")
        lines.append("")
...
async def search_sathi_seed_availability(
    ...
    """SATHI certified seed availability for crop_code at latitude, longitude."""
